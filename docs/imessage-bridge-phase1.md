...
Each entry binds an exact `chat.guid` to an exact set of participant handles and safe display labels. Handles are used only to validate membership and map inbound senders; returned results contain configured display labels rather than phone numbers or Apple IDs. If group membership changes, access fails closed until Tyler reviews and updates the allowlist.

## MCP tools

All tools declare `readOnlyHint: true`, `destructiveHint: false`, `idempotentHint: true`, and `openWorldHint: false`. Every schema has `additionalProperties: false`.

### `list_allowed_message_threads`

Input: an empty object.

Output: the configured conversation identifier, safe display name, and configured participant display names for allowlisted threads whose current membership still matches.
...
```json
{
  "conversationId": "exact-allowlisted-chat-guid",
  "limit": 20
}
```
...
4. Do not open `chat.db` in a writable database browser. From the approved terminal, use the system SQLite client in read-only mode to identify one non-sensitive test thread by a known synthetic phrase or exact known participant handle. Keep output metadata-only and bounded. For example, replace `KNOWN_TEST_HANDLE` locally and run:

   ```sh
   /usr/bin/sqlite3 -readonly "$HOME/Library/Messages/chat.db" \
     "SELECT c.guid, COUNT(*) AS participant_count FROM chat c JOIN chat_handle_join chj ON chj.chat_id=c.ROWID JOIN handle h ON h.ROWID=chj.handle_id WHERE h.id = 'KNOWN_TEST_HANDLE' GROUP BY c.ROWID, c.guid ORDER BY c.ROWID DESC LIMIT 5;"
   ```
...
5. Query only that selected chat's participant metadata, with `SELECT h.id ... WHERE c.guid = 'SELECTED_GUID'`, to build one exact allowlist entry. Do not query `message.text` during discovery.
...
7. Start `node bin/imessage-readonly-mcp.js` locally over stdio. Call `list_allowed_message_threads`, confirm it returns exactly one thread, then call `read_recent_messages` with that conversation ID and `limit: 5`. Do not search or request more history in the first validation.
